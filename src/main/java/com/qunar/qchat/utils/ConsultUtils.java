...
public class ConsultUtils {
    ...
    public static boolean sendMessage(JID from, JID to, JID realfrom, JID realto, String message, boolean toSeat, boolean isImage) {
        String messageParam = createConsultMessage(message, from.toBareJID(), realfrom.toBareJID(), to.toBareJID(), realto.toBareJID(),toSeat, isImage);
    ...
    private static boolean sendThirdMessage(String from, String to, String message) {
        ...
        if (!Strings.isNullOrEmpty(response)) {
            JsonResult qChatResult = JacksonUtils.string2Obj(response, JsonResult.class);
